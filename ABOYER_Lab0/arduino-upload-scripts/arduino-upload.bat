...
rem This awful for loop is the only way to do command redirection in windows cli apparently...
rem the delims= is to remove the effect of spaces
for /f "delims=" %%i in ('dir /s /b "%~dp0*%2" ^| findstr "BUILD" ^| findstr "ARDUINO_NANO33BLESENSE"') do (
	set bin_to_load=%%i
)
